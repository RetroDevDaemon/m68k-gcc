...
void UnflashAllPalettes()
{
    for (u8 i = 0; i < 64; i++){
        ...
        u8 r = (p1 & 0b1111);
        // if it does not equal the value in the palette
        if(*(*(curPaletteSet[(u8)(i/16)]) + i) != p1){
            // extract out target RGB
            u16 p2 = *(*(curPaletteSet[0]) + i);
            u16 tb = (p2 & 0b111100000000) >> 8;
            u16 tg = (p2 & 0b11110000) >> 4;
            u8 tr = (p2 & 0b1111);
            // go one by one
            if (b > tb) b = max(b - 2, 0);
            if (g > tg) g = max(g - 2, 0);
